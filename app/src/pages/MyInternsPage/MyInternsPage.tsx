...
import { useInternsByCompany } from "@/hooks";
...
const MyInternsPage = () => {
  ...
  const { data, isPending } = useInternsByCompany();
    ...
    if (searchTerm) {
      setDisplayedInterns([
        ...data.filter((intern: Intern) => {
          return intern.firstName
            .concat(" ".concat(intern.lastName))
            .toLowerCase()
  ...
  return (
    <Box>
      <Typography sx={{ marginY: "10px", fontSize: 25 }}>My Interns</Typography>
      <form
